...
主页面的“星历定位”支持小时级公历时间，也支持天文年日期输入：`0000` 表示 1 BC，`-0199` 表示公元前 200 年，例如 `-0199-04-08 20:00`。历史日期可选择自动历法（1582-10-15 前使用儒略历）、回溯格里高利历或全程儒略历。

日期输入范围扩展到当前深时模型约 ±5 万年。输入的 UTC/CST 先通过 NASA/Espenak 的 `ΔT = TT - UT` 近似转换为 TT，再由 JPL SSD 近似根数和开普勒模型计算；这不是 DE440/DE441 精密星历。现代日期适合可视化和小时级位置展示，古代日期还会受到历史 `ΔT` 不确定性的影响，应视为近似外推。详细算法、HORIZONS 对照误差和每轮实现记录见 [`analysis-records/`](analysis-records/)。
