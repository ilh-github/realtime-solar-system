;(function(root,factory){
  ...
  var Lunar = (function(){
    ...
    var _computeMonth = function(o,solar){
      ...
      var index = -3;
      // 修正: 用儒略日数值比较替代字符串比较, 修复负年份(公元前)跨年排序反转导致的月柱错误
      var solarDayJd = Math.floor(solar.getJulianDay() + 0.5);
      for(i=0;i<size;i+=2){
        end = o.jieQi[LunarUtil.JIE_QI_IN_USE[i]];
        var symdJd = null==start?solarDayJd:Math.floor(start.getJulianDay()+0.5);
        var endDayJd = Math.floor(end.getJulianDay()+0.5);
        if(solarDayJd>=symdJd&&solarDayJd<endDayJd){
          break;
        }
        start = end;
        index++;
      ...
      index = -3;
      // 修正: 用儒略日数值比较替代字符串比较, 修复负年份(公元前)跨年排序反转导致的月柱错误
      var solarJd = solar.getJulianDay();
      for(i=0;i<size;i+=2){
        end = o.jieQi[LunarUtil.JIE_QI_IN_USE[i]];
        var stimeJd = null==start?solarJd:start.getJulianDay();
        if(solarJd>=stimeJd&&solarJd<end.getJulianDay()){
          break;
        }
        start = end;
        index++;
    ...
    var _computeDay = function(o,solar,hour,minute){
      ...
      // 修正: 取正模, 修复负儒略日(公元前4713年前)日柱 index 为负导致 undefined
      var dayGanIndex = ((offset % 10) + 10) % 10;
      var dayZhiIndex = ((offset % 12) + 12) % 12;
